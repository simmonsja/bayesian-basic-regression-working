...
We could also frame this in another way that is perhaps more Bayesian and will align better with our code below. We could say that our observed values of $\Delta W$ are generated from a normal distribution with mean $\mathbf{X} \mathbf{\beta}$ and standard deviation $\sigma$. Same emperor, new clothes.

$$
\Delta W \sim \mathcal{N}(\mathbf{X} \mathbf{\beta}, \sigma)
$$

## A brief aside on fitting

But, how the heck does this help us to fit a model? We're not going to be able to answer this fully, but lets build some inituition. We need to fit the parameters of our model ($\mathbf{\beta}$ and $\sigma$).
