...
scale_max_vals = df.max()
scale_min_vals = df.min()
...
fit_smoothie = mod_smoothie.fit(
    draws = 2000
)
# and make predcictions (added to fit_smoothie)
mod_smoothie.predict(fit_smoothie, kind = "response")
```
...
3. the next part is specifying the form of the residuals or the distribution of observations given the model as we saw above. this defines our likelihood(XX check Vs the stan definition which is different I think as it includes the prior) as discussed above. 
...
ax1 = az.plot_forest(
    fit_smoothie,
    var_names=["~sigma", "~Intercept"],
    combined = True,
    hdi_prob = 0.89,
    figsize = (5, 6)
)
# loop through each y axis tick label and make it subscript the location
ax1[0].set_yticklabels([_.get_text().replace("[", "$_{[").replace("]", "]}$") for _ in ax1[0].get_yticklabels()])
# set x axis gridlines as grey
ax1[0].grid(axis="x", color="lightgrey")
plt.show()
```
...
# rescale dW, dW_pred and dW_paper to the original scale
for col in ["dW", "dW_pred", "dW_paper"]:
    plot_df[col] = plot_df[col] * (scale_max_vals["dW"] - scale_min_vals["dW"]) + scale_min_vals["dW"]

# calcualte the R2 for each of dW_pred and dW_paper per location
r2_pred = pd.DataFrame()
r2_paper = pd.DataFrame()
rmse_pred = pd.DataFrame()
rmse_paper = pd.DataFrame()
for this_loc in df["location"].unique():
    this_df = plot_df.loc[plot_df["location"] == this_loc]
    r2_pred.loc[this_loc, "r2"] = r2_score(this_df["dW"], this_df["dW_pred"])
    r2_paper.loc[this_loc, "r2"] = r2_score(this_df["dW"], this_df["dW_paper"])
    rmse_pred.loc[this_loc, "rmse"] = root_mean_squared_error(this_df["dW"], this_df["dW_pred"])
    rmse_paper.loc[this_loc, "rmse"] = root_mean_squared_error(this_df["dW"], this_df["dW_paper"])
# print(r2_pred)

# plot a scatter that is facetted by location
g = sns.FacetGrid(plot_df, col="location", col_wrap=3, height=3)
g.map(sns.scatterplot, "dW", "dW_pred")
g.set_axis_labels("Observed dW", "Predicted dW")
# red dashed 1:1 line
for ax in g.axes:
    ax.plot([plot_df["dW"].min(), plot_df["dW"].max()], [plot_df["dW"].min(), plot_df["dW"].max()], color="red", linestyle="--", alpha=0.5)
# print the R2 values in top left of each plot per location
for ax, r2 in zip(g.axes, r2_pred["r2"].values):
...
for ax, rmse in zip(g.axes, rmse_pred["rmse"].values):
    ax.text(0.05, 0.85, f"RMSE: {rmse:.2f}", transform=ax.transAxes, verticalalignment="top", fontsize=10)

# now plot the same for the paper predictions in df["dW_pred"]

g = sns.FacetGrid(plot_df, col="location", col_wrap=3, height=3)
g.map(sns.scatterplot, "dW", "dW_paper")
g.set_axis_labels("Observed dW", "Predicted dW")
# red dashed 1:1 line
for ax in g.axes:
    ax.plot([plot_df["dW"].min(), plot_df["dW"].max()], [plot_df["dW"].min(), plot_df["dW"].max()], color="red", linestyle="--", alpha=0.5)
# print the R2 values in top left of each plot per location
for ax, r2 in zip(g.axes, r2_paper["r2"].values):
    ax.text(0.05, 0.95, f"R2: {r2:.2f}", transform=ax.transAxes, verticalalignment="top", fontsize=10)
# print the RMSE values
for ax, rmse in zip(g.axes, rmse_paper["rmse"].values):
    ax.text(0.05, 0.85, f"RMSE: {rmse:.2f}", transform=ax.transAxes, verticalalignment="top", fontsize=10)

plt.show()
```

Hey! We're not doing to bad at all, but something's off at the partially sheltered locations.
...
but bayesy you ain't seen nothing yet. the real power of Bayes comes from the ability of this workflow to scale to much much more complex models. the extra degree of freedom given by the prior, allows us to fully describe our beliefs about the data in a way that we only glimpsed in this example.
