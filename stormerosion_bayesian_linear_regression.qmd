...
But, how the heck does this help us to fit a model? What do fit even mean in a Bayesian framework?

In a Bayesian framework we are not aiming to learn a single set of "true" parameter values for our $\beta$'s and $\sigma$. That would be so very frequentist, instead we treat parameters as being probabilistic. Maybe there's a true value to be found, theoretically if we had the perfect model and infinite data (environmental systems never play this nice 🥲). There's no reason to get more into frequentist vs Bayesian approaches, I only bring it up as most people default to frequentist approaches. Both are great, both are useful, both can quantify uncertainty even. But this is a post about Bayes and that'll be that.

To get to out posterior distribution we need three things:

- a model that includes a specified likelihood function
- prior distributions for each of our paramaters
- data

Our assumptions about the form of the model and the data generating process that we specified above give rise to a likelihood. The likelihood is a measure of how likely the data are given the model and a set of parameter values. By specifying 



We're not going to be able to answer this fully, but lets build some inituition. Using a Bayesian approach we are setting out to find the posterior distrbituion of our parameters ($\mathbf{\beta}$ and $\sigma$). 

Our machine for obtaining samples to build a picture of the posterior is going to be Markov Chain Monte Carlo (MCMC). For a great introduction to MCMC see [this lecture](https://youtu.be/rZk2FqX2XnY?si=UpGAmJ3LEGXtGSz-&t=610) from Richard McElreath's great Statistical Rethinking course.
...
# make dummy data y = 0.1 x + 0.2 + noise (0.1)
np.random.seed(0)
...
loglik = numpyro.infer.util.log_likelihood(
    model = model, 
    posterior_samples = proposed_solutions,
    X = X, 
    y = y
)
...
for i in range(len(loglik['y_out'])):
    ...
    plt.plot(X, X * proposed_solutions['beta_1'][i] + proposed_solutions['beta_0'][i], color=plt.cm.viridis(loglik['y_out'][i].mean()), alpha=0.5, lw = 2)
plt.xlabel("X")
plt.ylabel("y")
plt.colorbar(label="log likelihood")
plt.show()

print(loglik['y_out'].mean(axis=1))
```



For our purposes, for the rest of this blog we are going to treat the fitting process as a magic. Please go and read more from many better sources than I to build an understanding of MCMC. But for us. We specify the data generating process with our model. Then the NumPyro sampling fairy appears from nowhere, pries open the chassis of my laptop and greedily feeds on the power being fed to my CPU for a period of seconds to minutes. The gorged fairy then waves its magic wand and voilà, samples from the posterior distribution are delivered into memory.
